...
/**** move ****/
  ...
  Program.command('move')
    ...
    .option('--shift-to <deg>',  'shift head forward (>90°) or back (<90°) — s1')
    .option('--roll-to <deg>',   'roll head clockwise (>90°) or counter-clockwise (<90°) — s2')
    .option('--pitch-to <deg>',  'pitch head up (>110°) or down (<110°) — s3')
    .option('--rotate-to <deg>', 'rotate body around Z-axis — s4')
    .option('--lift-to <deg>',   'lift head on secondary axis, range 20°–150° — s5')
...
/**** shift-to / roll-to / pitch-to / rotate-to / lift-to ****/
  ...
  Program.command('shift-to')
    ...
    .argument('<deg>', 'target angle in degrees')
    .action(async (DegArg:string) => {
      const Nova = await getController()
      Nova.State = { s1:Number(DegArg) }
      await Nova.sendServoState()
    })
